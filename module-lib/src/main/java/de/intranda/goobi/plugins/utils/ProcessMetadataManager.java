...
public class ProcessMetadataManager extends ProcessManager {
    ...
    private List<Process> getProcessesFromDB(String order, String filter, Integer start, Integer count) {
        ...
        sql.append(
                "left join (select processid, value from metadata where name='TitleDocMain' group by processid) md1 on md1.processid = prozesse.ProzesseID ");
        sql.append(
                "left join (select processid, value from metadata where name='Author' group by processid) md2 on md2.processid = prozesse.ProzesseID ");
        sql.append(
                "left join (select processid, value from metadata where name='PublicationYear' group by processid) md3 on md3.processid = prozesse.ProzesseID ");
